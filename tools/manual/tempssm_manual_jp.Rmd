...
数式を含む詳細なモデル定式化は、英語版 vignette `model-specification` および
日本語版の`tools/manual/tempssm_model_jp.Rmd` を参照してください。
...
明瞭な季節変動パターンが認められます。この時系列には
`r sst_sim_na` 個の欠測値が含まれています。生の時系列からは、
...
`diagnose_residual_ts(resid)` を利用できます。いずれの場合も、検定に用いる
...
`plot_tempssm_model_residuals(res_ar1)` を利用できます。このラッパーは内部で
...
このモデルの残差診断でも、lag 12 までの有意な残差自己相関は認められません。
