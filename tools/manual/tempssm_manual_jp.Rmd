...
`diagnose_residuals()` を用います。検定に用いるラグは、デフォルトでは入力
データの季節周期に合わせており、月別データでは lag 12 を用います。推定済みの
`tempssm` オブジェクトを直接プロットしたい場合には、ワンコマンドのラッパー
`plot_tempssm_residual_diagnostics(res_ar1)` も利用できます。
...
`diagnose_residuals()` の返り値のうち、`lb_stat`、`lb_lag`、`lb_pvalue` は
