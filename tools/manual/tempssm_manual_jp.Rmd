...
月別データでは $f=12$）。この定式化により季節効果が識別可能となり、異なる
...
このような単発の突出が偶然に生じることもあるため、ACF プロットだけでモデル
変更を判断するのではなく、Ljung-Box 検定や残差時系列プロットとあわせて
解釈します。
...
SST の平均的な年上昇率は `r sprintf("%.4f", mean_drift_year)` °C と
推定されました。
