...
`tempssm` のコアとなるモデル推定関数では、入力する温度時系列および任意の
外生変数を、等間隔時系列を表す R の `ts` オブジェクトとして与えることを
前提としています（R 公式の
`stats::ts` ドキュメントを参照：
...
オブジェクトとして与える必要があります
（`?stats::ts` または
<https://search.r-project.org/R/refmans/stats/html/ts.html> を参照）。

データ準備を支援するため、本パッケージには、外部の観測データを
`ts` オブジェクトへ変換するユーティリティ関数が含まれています
（付録を参照）。
