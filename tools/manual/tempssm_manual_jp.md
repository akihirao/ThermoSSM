...
次節では、モデルの概要のみを簡潔に整理します。数式を含む詳細な定式化は、
英語版 vignette `model-specification` および日本語版の
`tools/manual/tempssm_model_jp.Rmd` に分離しています。
...
数式を含む詳細なモデル定式化は、英語版 vignette `model-specification`
および 日本語版の`tools/manual/tempssm_model_jp.Rmd`
を参照してください。
観測方程式、状態方程式、季節成分の制約、自己回帰成分、外生成分、
推定パラメータ数、推定手順の詳細を説明しています。
...
場合には、少数のラグが偶然に信頼限界を超えることもあります。一方で、周期的
または連続的な突出がみられる場合には、時間構造が残っている可能性があります。
