...
`tempssm` は、気温・水温など、環境中で観測される温度時系列に対して
状態空間モデルを適用するための R
パッケージです。長期トレンド、季節変動、
自己回帰的依存性、および任意の外生効果が、観測された時間変動にどのように
寄与しているかを評価するための実用的な枠組みを提供します。計算バックエンド
として `KFAS` R パッケージ（Helske, 2017）を用い、カルマンフィルタリング
および平滑化によって推定される線形ガウス状態空間モデルを適用します。
...
R の `ts` オブジェクトは、`tempssm` の主要な入力データ形式です。
`tempssm()` に渡す温度時系列は、単変量の `ts` オブジェクトとして
...
季節成分の制約、自己回帰成分、外生成分、推定パラメータ数、推定手順については、
英語版 vignette `model-specification` および日本語版の
