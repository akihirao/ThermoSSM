...
プロットには `ggplot2`、一部の時系列変換ユーティリティには `zoo`、
チュートリアルでの基本的な時系列可視化には `forecast` を用いています。
...
一つです。馬場ら (2024) による温度時系列への適用を踏まえ、`tempssm`
...
まず、月別 SST
時系列を可視化し、見かけのトレンド、季節変動、欠測値の有無を含む
全体的な構造を確認しておきます。
...
![](tempssm_manual_jp_files/figure-html/unnamed-chunk-3-1.png)
...
本パッケージのコア関数[`tempssm()`](https://akihirao.github.io/tempssm/reference/tempssm.md)に温度時系列データの
`ts` オブジェクト （ここでは
...
オブジェクトに保存され、[`logLik()`](https://rdrr.io/r/stats/logLik.html)
や [`summary()`](https://rdrr.io/r/base/summary.html) などの下流メソッド
でも一貫して用いられます。
...
[`logLik()`](https://rdrr.io/r/stats/logLik.html) から 取得できます。
...
![](tempssm_manual_jp_files/figure-html/unnamed-chunk-10-1.png)

ここでは、[`get_tempssm_residuals()`](https://akihirao.github.io/tempssm/reference/get_tempssm_residuals.md)
によって標準化再帰残差を取り出し、
...
オブジェクトとして残差を返します。欠損値または非有限の残差は `NA` として
...
あります。[`get_tempssm_residuals()`](https://akihirao.github.io/tempssm/reference/get_tempssm_residuals.md)
によって時間軸を保持した残差を確認する
...
取り出した残差系列の可視化には `plot_tempssm_residuals(resid)`
を用います。 この関数により、残差時系列プロット、ACF
プロット、残差の頻度分布を 確認できます。Ljung-Box
...
推定済みの `tempssm`
オブジェクトは、[`predict()`](https://rdrr.io/r/stats/predict.html)
に渡すことで短期予測を 取得できます。デフォルトでは、`predict(res)`
は観測期間の終了直後の 1
...
![](tempssm_manual_jp_files/figure-html/unnamed-chunk-21-1.png)

ここで用いるシミュレーション外生変数には欠測値は含まれていません。
ユーザー自身の外生変数データを用いる場合、欠測値はモデル当てはめの前に
補完する、欠測期間を除外する、または解析期間を調整するなどして、
外生変数に `NA`
が残らない形に整えてください。[`tempssm()`](https://akihirao.github.io/tempssm/reference/tempssm.md)
は外生変数の 欠測値を許容しません。一方で、
...
[`tempssm()`](https://akihirao.github.io/tempssm/reference/tempssm.md)
の `exo_data` 引数には、単変量 `ts` オブジェクトまたは 多変量 `ts`
...
![](tempssm_manual_jp_files/figure-html/unnamed-chunk-24-1.png)
...
![](tempssm_manual_jp_files/figure-html/unnamed-chunk-28-1.png)
...
![](tempssm_manual_jp_files/figure-html/unnamed-chunk-29-1.png)
...
関数は、このような CSV ファイルを読み込み、
[`tempssm()`](https://akihirao.github.io/tempssm/reference/tempssm.md)
で利用できる R の `ts` オブジェクトへ変換します。
より細かな制御が必要な場合は、手動で `ts` オブジェクトを作成することも
できます。詳細は R 公式の [`stats::ts`](https://rdrr.io/r/stats/ts.html)
ドキュメントを参照してください：
<https://search.r-project.org/R/refmans/stats/html/ts.html>。
...
`tempssm` に実装されている統計モデリングの枠組みは、馬場ら (2024)
に記述された方法論に基づいており、付随する補足資料および
コードリポジトリが本パッケージ開発の初期ソースとなっています。

馬場真哉, 石井 洋, 芳山 拓. (2024).
線形ガウス状態空間モデルを用いた神奈川県城ヶ島地先における海水温トレンドの推定.
*水産海洋研究*, 88(3), 190–199. <https://doi.org/10.34423/jsfo.88.3_190>
...
Helske, J. (2017). KFAS: KFAS: Exponential family state space models in
...
[`read_monthly_temp_ts()`](https://akihirao.github.io/tempssm/reference/read_monthly_temp_ts.md)
関数は、CSV ファイルに保存された月別温度データを R の `ts`
...
ファイルを準備します。デフォルトでは 、列名は `Year`、`Month`、`Temp`
である必要があります。ここで `Temp` は 観測された温度値を表します。
...
入力データフレームには、少なくとも日付列（`Date`）と温度列（`Temp`）の 2
...
関数は、R の `ts` オブジェクトとして与えられた
時系列から偏差を計算します。偏差は、各観測値から対応する周期単位（たとえば月）の
長期平均を差し引くことで計算され、周期の影響を取り除いた年変動パターンの確認
などに用いられます。

偏差を計算するための基準期間は、関数引数で指定できます。デフォルトでは、利用
可能な時系列全体を用いて平年値が計算されます（[`?compute_temp_anomaly`](https://akihirao.github.io/tempssm/reference/compute_temp_anomaly.md)
を参照）。
...
![](tempssm_manual_jp_files/figure-html/unnamed-chunk-35-1.png)
...
関数は、`ts` オブジェクトから、各季節値を
年をまたいで平均することにより、気候値 (climatology:
気候学的な平均季節周期) を
...
![](tempssm_manual_jp_files/figure-html/unnamed-chunk-36-1.png)
