...
#' names `level`, `drift`, `season`, and `ar`.
#'
...
#' `plot_tempssm_components()`, so `plot(res)` displays the same component
#' plots as the explicit component-plot helper and accepts the same plotting
#' arguments. The ggplot2-style S3 method `autoplot.tempssm()` is also retained
#' for users who prefer the ggplot2 generic.
#'
...
#' \code{tempssm()}. This function returns the same \code{ggplot} object as
#' \code{autoplot.tempssm()} and is useful when a descriptive function name is
#' preferred in scripts or examples.
#'
#' @param x An object returned by \code{tempssm()}.
#' @inheritParams autoplot.tempssm
#'
#' @return
#' A \code{ggplot} object. Multiple components are represented as facets with
#' free y-axis scales and component-specific units in the facet labels.
...
#' This method delegates to \code{plot_tempssm_components()} so that
#' \code{plot(res)} displays the same component plots as the explicit helper.
#'
#' @param x An object returned by \code{tempssm()}.
#' @param ... Additional arguments passed to \code{plot_tempssm_components()}.
#'
#' @return
