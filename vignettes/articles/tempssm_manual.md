...
residuals are represented as `NA`. For calculations that require a
complete numeric vector, use
...
exogenous series. `tempssm()` does not allow missing exogenous values.
In contrast, missing values in the dependent temperature series are
allowed and are treated as unobserved responses during Kalman filtering
and smoothing.
